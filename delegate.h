...
public:
  ...
  template <class C> delegate(C &object, R (C::*const method_ptr)(A...)) {
  ...
  template <typename T> static delegate from(T &&f) {
...
private:
  ...
  template <class T> static void deleter_stub(void *const p) {
    static_cast<T *>(p)->~T();
  }
