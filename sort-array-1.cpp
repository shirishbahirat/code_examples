...
void quick_sort(uint32_t arr[], uint32_t low,
                uint32_t high); // https://en.wikipedia.org/wiki/Quicksort
...
uint32_t partation(uint32_t arr[], uint32_t low, uint32_t high) {
  ...
  uint32_t index =
      low; // if array element is less that pivot then only increment index
