...
def q_learning_keras(env, num_episodes=100):
    ...
    for i in range(5):
        ...
        q_table[i] = model.predict(np.identity(5)[i:i + 1])
...
def test_methods(env, num_iterations=10):
    ...
    for g in range(num_iterations):
        m0_table = naive_sum_reward_agent(env, 10)
        m1_table = q_learning_with_table(env, 10)
        m2_table = eps_greedy_q_learning_with_table(env, 10)
        m3_table = q_learning_keras(env, 10)
